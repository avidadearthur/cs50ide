...
bool vote(int voter, int rank, string name)
{
    ...
    bool exists = false;
    int candIndex;
    // first of all check if the candidate name the ith
    // voter chose for the jth rank exists
    ...
    // if it exists, yhen record the vote
    if (exists)
    {
        // store the candidate index from the array of candidates
        // in the array of preferences or ballots
        preferences[voter][rank] = candIndex;
    }
...
int find_min(void)
{
    ...
    for (int i = 1; i < candidate_count; i++)
    {
        ...
        if (looser.votes > candidates[i].votes && !candidates[i].eliminated)
        {
            looser = candidates[i];
        }
    }
